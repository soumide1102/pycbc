...
def kullback_leibler(logposterior_array, logprior_array,
    ...
    for ii in range(len(starts)):
        ...
        logposterior=logposterior_array[:end]
        logprior=logprior_array[:end]
        #posterior_scaled=posterior_array[:end]
        l = len(logposterior)
        stat = 0.0
        for jj in range(l):
            print("logposterior[jj]", logposterior[jj])
            print("logprior[jj]", logprior[jj])
            #log_ratio = 1000.0*(logposterior_scaled[jj] - logprior_scaled[jj])
            log_ratio = (logposterior[jj] - logprior[jj])
            ...
            stat = stat + numpy.exp(logposterior[jj] + numpy.log(log_ratio))
